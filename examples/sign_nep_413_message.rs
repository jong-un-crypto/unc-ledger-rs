...
use unc_ledger::{UNCLedgerError, NEP413Payload};
use unc_primitives::signable_message::{MessageDiscriminant, SignableMessage};
use unc_primitives_core::{borsh, hash::CryptoHash};
...
fn main() -> Result<(), UNCLedgerError> {
    ...
    let public_key = unc_ledger::get_public_key_with_display_flag(hd_path.clone(), false)?;
    ...
    let msg = NEP413Payload {
        ...
        nonce: [42; 32],
        recipient: "alice.unc".to_string(),
        callback_url: Some("myapp.com/callback".to_string()) 
    };

    let signature_bytes = unc_ledger::sign_message_nep413(&msg, hd_path)?;
